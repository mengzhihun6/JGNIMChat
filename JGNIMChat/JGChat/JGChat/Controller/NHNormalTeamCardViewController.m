...
//
//  NHNormalTeamCardViewController.m
//  NestHouse
//
...
#import "NHNormalTeamCardViewController.h"
...
#import "NHSessionChatHistoryController.h"//聊天历史记录
...
@interface NHNormalTeamCardViewController ()<NIMTeamManagerDelegate, NIMTeamMemberCardActionDelegate,UITableViewDataSource,UITableViewDelegate,NIMTeamSwitchProtocol,NIMContactSelectDelegate,NHMemberGroupViewDelegate>{
...
@property (nonatomic,strong) NHMemberGroupView *headerView;
...
@property (nonatomic,strong) NIMSession *session;   //聊天记录查session

@end

@implementation NHNormalTeamCardViewController
...
- (NSArray*)buildBodyData{
    ...
    NIMTeamCardRowItem *searchHistory = [[NIMTeamCardRowItem alloc]init];
    searchHistory.title            = @"查询聊天记录";
    searchHistory.action      = @selector(searchChatHistory);
    searchHistory.rowHeight  = 50.f;
    searchHistory.type        = TeamCardRowItemTypeCommon;
    
    
    NIMTeamCardRowItem *dropOff = [[NIMTeamCardRowItem alloc]init];
    dropOff.title            = @"清空聊天记录";
    dropOff.action      = @selector(dropOffHistory);
    dropOff.rowHeight  = 50.f;
    dropOff.type        = TeamCardRowItemTypeCommon;
    ...
    return @[@[itemName],@[searchHistory],@[dropOff],@[teamNotify],@[itemQuit]];
}
...
- (void)updateTeamInfoName{
    _updateTeamNameAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"修改群组名称" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
    _updateTeamNameAlertView.alertViewStyle = UIAlertViewStylePlainTextInput;
    [_updateTeamNameAlertView show];
}

- (void)quitTeam{
    _quitTeamAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"确认退出群?" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确认", nil];
...
- (void)setupTableHeader:(CGFloat)width{
    self.headerView = [[NHMemberGroupView alloc] initWithFrame:CGRectZero];
