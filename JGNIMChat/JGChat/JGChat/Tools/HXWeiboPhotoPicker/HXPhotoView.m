...
//    [[self viewController:self] presentViewController:[[UINavigationController alloc] initWithRootViewController:vc] animated:YES completion:nil];
    ...
    if ([_delegate respondsToSelector:@selector(pushIntoPhotoController)]) {
        [_delegate pushIntoPhotoController];
    }
...
- (void)setupNewFrame
{
    ...
    //cell高度和宽度
    self.flowLayout.itemSize = CGSizeMake(W(80), H(80));
    self.flowLayout.minimumLineSpacing = Spacing;
    
        self.frame = CGRectMake(x, y, self.dataList.count * (W(80) + 20) , H(80));
